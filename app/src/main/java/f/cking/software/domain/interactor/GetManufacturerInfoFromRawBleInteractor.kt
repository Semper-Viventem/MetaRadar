...
class GetManufacturerInfoFromRawBleInteractor(
    ...
    fun execute(
        ...
        val frame: BleRecordFrame =
            getBleRecordFramesFromRawInteractor.execute(raw).firstOrNull {
                it.type == TYPE_MANUFACTURER_INFO
            } ?: return null

        return frame.data
            .takeIf { it.count() >= MIN_MANUFACTURER_ID_LENGTH }
            ?.let { data ->
    ...
    private fun checkAirdrop(
        ...
        if (manufacturerId == ManufacturerInfo.APPLE_ID) {
