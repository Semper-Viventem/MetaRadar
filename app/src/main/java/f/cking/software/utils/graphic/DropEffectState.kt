...
@Composable
fun Modifier.withDropEffect(dropEffectState: DropEffectState): Modifier =
    composed {
...
class DropEffectState {
    ...
    data class DropEvent(
        ...
        enum class Type(
            val animSpecs: List<AnimationSpec>,
            val postFactor: Float? = null,
        ) {
            ...
            RELEASE_HARD(
                animSpecs =
                    listOf(
                        AnimationSpec(to = 0f, duration = 50),
                        AnimationSpec(to = 2f, duration = 1000, easing = CubicBezierEasing(0f, 0.5f, 0.75f, 1f)),
                    ),
                postFactor = 0f,
            ),
        }
...
@Composable
fun rememberDropEffectState(): DropEffectState = remember { DropEffectState() }
