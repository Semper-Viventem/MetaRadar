...
//        }
    ...
    private fun checkBackgroundWarningIsExpired(hideMessageTime: Long): Boolean =
        System.currentTimeMillis() - hideMessageTime > TimeUnit.DAYS.toMillis(3)
    ...
    private fun filterQuery(
        ...
        query?.takeIf { it.isNotBlank() }?.let { searchStr ->
    ...
    private suspend fun checkFilter(
        device: DeviceData,
    ...
    object DefaultFilters {
        fun notApple(context: Context) =
            FilterHolder(
                displayName = context.getString(R.string.not_apple),
                filter =
                    RadarProfile.Filter.Not(
    ...
    enum class ActiveScannerExpandedState {
        ...
        fun next(): ActiveScannerExpandedState = entries.elementAt((ordinal + 1) % entries.size)
    ...
    companion object {
        ...
        private val GENERAL_COMPARATOR =
            Comparator<DeviceData> { second, first ->
                ...
                when {
                    ...
                    first.resolvedName != second.resolvedName ->
                        first.resolvedName?.compareTo(second.resolvedName ?: return@Comparator 1)
                            ?: -1
