...
object DeviceListScreen {
    ...
    fun Screen() {
        ...
        if (viewModel.devicesViewState.isEmpty() &&
            !viewModel.isSearchMode &&
            appliedFilter.isEmpty() &&
            viewModel.currentBatchViewState == null
        ) {
    ...
    @Composable
    fun CurrentBatch(viewModel: DeviceListViewModel) {
        Spacer(modifier = Modifier.height(8.dp))
    ...
    @Composable
    fun rememberAnimatedVectorPainterCompat(
    ...
    private fun CurrentBatchList(viewModel: DeviceListViewModel) {
        ...
        val visibleDevices =
            when (mode) {
                DeviceListViewModel.ActiveScannerExpandedState.COLLAPSED ->
                    currentBatch.take(
                        DeviceListViewModel.ActiveScannerExpandedState.MAX_DEVICES_COUNT,
                    )
        ...
        RoundedBox(Modifier.padding(horizontal = 16.dp, vertical = 8.dp)) {
            ...
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Button(
                    ...
                    onClick = viewModel::onHideBackgroundLocationWarningClick,
                    colors =
    ...
    @Composable
    private fun EnjoyTheApp(
        viewModel: DeviceListViewModel,
        enjoyTheAppState: DeviceListViewModel.EnjoyTheAppState,
    ) {
        RoundedBox(Modifier.padding(horizontal = 16.dp, vertical = 8.dp)) {
    ...
    private fun EnjoyTheAppQuestion(viewModel: DeviceListViewModel) {
        Column {
            ...
            Row {
                Button(
                    modifier = Modifier.weight(1f),
                    onClick = { viewModel.onEnjoyTheAppAnswered(DeviceListViewModel.EnjoyTheAppAnswer.LIKE) },
                ) {
                ...
                Button(
                    modifier = Modifier.weight(1f),
                    onClick = { viewModel.onEnjoyTheAppAnswered(DeviceListViewModel.EnjoyTheAppAnswer.DISLIKE) },
                ) {
    ...
    @Composable
    private fun EnjoyTheAppLike(
        state: DeviceListViewModel.EnjoyTheAppState.Like,
        viewModel: DeviceListViewModel,
    ) {
        Column {
        ...
        SelectFilterTypeScreen.Dialog(selectFilterDialog) { initialFilter ->
            viewModel.router.navigate(
                ScreenNavigationCommands.OpenCreateFilterScreen(
                    ...
                    val filterHolder =
                        DeviceListViewModel.FilterHolder(
