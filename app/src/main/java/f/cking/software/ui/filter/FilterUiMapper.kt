...
object FilterUiMapper {
    fun mapToDomain(from: FilterUiState): RadarProfile.Filter =
        when (from) {
            ...
            is FilterUiState.LastDetectionInterval ->
                RadarProfile.Filter.LastDetectionInterval(
                    from = mapTimeToUi(from.fromDate, from.fromTime, Long.MIN_VALUE),
                    to = mapTimeToUi(from.toDate, from.toTime, Long.MAX_VALUE),
                )

            is FilterUiState.FirstDetectionInterval ->
                RadarProfile.Filter.FirstDetectionInterval(
                    from = mapTimeToUi(from.fromDate, from.fromTime, Long.MIN_VALUE),
                    to = mapTimeToUi(from.toDate, from.toTime, Long.MAX_VALUE),
                )

            is FilterUiState.MinLostTime -> RadarProfile.Filter.MinLostTime(from.minLostTime!!)
            is FilterUiState.AppleAirdropContact ->
                RadarProfile.Filter.AppleAirdropContact(
            ...
            is FilterUiState.IsFollowing ->
                RadarProfile.Filter.IsFollowing(
                    followingDurationMs = from.followingDurationMs,
                    followingDetectionIntervalMs = from.followingDetectionIntervalMs,
                )

            is FilterUiState.DeviceLocation ->
                RadarProfile.Filter.DeviceLocation(
            ...
            is FilterUiState.UserLocation ->
                RadarProfile.Filter.UserLocation(
                    location = from.targetLocation!!,
                    radiusMeters = from.radius,
    ...
    fun mapToUi(from: RadarProfile.Filter): FilterUiState =
        when (from) {
            is RadarProfile.Filter.Name ->
                FilterUiState.Name().apply {
                    this.name = from.name
                    this.ignoreCase = from.ignoreCase
                }

            is RadarProfile.Filter.Address ->
                FilterUiState.Address().apply {
                    this.address = from.address
                }

            is RadarProfile.Filter.Manufacturer ->
                FilterUiState.Manufacturer().apply {
                    this.manufacturer =
            ...
            is RadarProfile.Filter.IsFavorite ->
                FilterUiState.IsFavorite().apply {
                    this.favorite = from.favorite
                }

            is RadarProfile.Filter.IsPaired ->
                FilterUiState.IsPaired().apply {
                    this.isPaired = from.isPaired
                }

            is RadarProfile.Filter.FirstDetectionInterval ->
                FilterUiState.FirstDetectionInterval().apply {
            ...
            is RadarProfile.Filter.LastDetectionInterval ->
                FilterUiState.LastDetectionInterval().apply {
                    this.fromDate = from.from.takeIf { it != Long.MIN_VALUE }?.toLocalDate()
                    this.fromTime = from.from.takeIf { it != Long.MIN_VALUE }?.toLocalTime()
                    this.toDate = from.to.takeIf { it != Long.MAX_VALUE }?.toLocalDate()
                    this.toTime = from.to.takeIf { it != Long.MAX_VALUE }?.toLocalTime()
            ...
            is RadarProfile.Filter.MinLostTime ->
                FilterUiState.MinLostTime().apply {
                    this.minLostTime = from.minLostTime
                }

            is RadarProfile.Filter.ByTag ->
                FilterUiState.Tag().apply {
                    this.tag = from.tag
                }

            is RadarProfile.Filter.All ->
                FilterUiState.All().apply {
                    this.filters = from.filters.map { mapToUi(it) }
                }

            is RadarProfile.Filter.Any ->
                FilterUiState.Any().apply {
                    this.filters = from.filters.map { mapToUi(it) }
                }

            is RadarProfile.Filter.Not ->
                FilterUiState.Not().apply {
                    this.filter = mapToUi(from.filter)
                }

            is RadarProfile.Filter.AppleAirdropContact ->
                FilterUiState.AppleAirdropContact().apply {
                    this.contactString = from.contactStr
                    this.minLostTime = from.minLostTime
                }

            is RadarProfile.Filter.IsFollowing ->
                FilterUiState.IsFollowing().apply {
                    this.followingDurationMs = from.followingDurationMs
                    this.followingDetectionIntervalMs = from.followingDetectionIntervalMs
                }

            is RadarProfile.Filter.DeviceLocation ->
                FilterUiState.DeviceLocation().apply {
            ...
            is RadarProfile.Filter.UserLocation ->
                FilterUiState.UserLocation().apply {
                    this.targetLocation = from.location
                    this.radius = from.radiusMeters
